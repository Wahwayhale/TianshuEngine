...
DepthOfFieldSystem::DepthOfFieldSystem(Device& device, uint32_t width, uint32_t height)
    : m_device(device), m_width(width), m_height(height) {

    createResources();
    SPARK_CORE_INFO("Depth of field system initialized: {0}x{1}", width, height);
}

DepthOfFieldSystem::~DepthOfFieldSystem() {
    vkDeviceWaitIdle(m_device.getDevice());
    cleanupResources();
    SPARK_CORE_INFO("Depth of field system destroyed.");
}

void DepthOfFieldSystem::recreate(uint32_t width, uint32_t height) {
    vkDeviceWaitIdle(m_device.getDevice());
    m_width = width;
    m_height = height;
...
void DepthOfFieldSystem::render(VkCommandBuffer commandBuffer, VkImageView colorView, VkImageView depthView) {
...
void DepthOfFieldSystem::createResources() {
    // 创建输出纹理
    ...
    SPARK_CORE_INFO("Depth of field resources created.");
}

void DepthOfFieldSystem::cleanupResources() {

...
VolumetricFogRenderer::VolumetricFogRenderer(Device& device, uint32_t width, uint32_t height)
    : m_device(device), m_width(width), m_height(height) {

    createResources();
    SPARK_CORE_INFO("Volumetric fog renderer initialized: {0}x{1}", width, height);
}

VolumetricFogRenderer::~VolumetricFogRenderer() {
    vkDeviceWaitIdle(m_device.getDevice());
    cleanupResources();
    SPARK_CORE_INFO("Volumetric fog renderer destroyed.");
}

void VolumetricFogRenderer::recreate(uint32_t width, uint32_t height) {
    vkDeviceWaitIdle(m_device.getDevice());
    m_width = width;
    m_height = height;
...
void VolumetricFogRenderer::render(VkCommandBuffer commandBuffer, VkImageView depthView,
...
void VolumetricFogRenderer::createResources() {
    // 创建输出纹理
    ...
    // 创建 UBO
    m_uboBuffer = std::make_unique<Buffer>(m_device, sizeof(VolumetricFogSettings),
                                            VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    m_uboBuffer->map();

    SPARK_CORE_INFO("Volumetric fog resources created.");
}

void VolumetricFogRenderer::cleanupResources() {

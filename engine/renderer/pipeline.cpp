...
Pipeline::Pipeline(Device& device, VkRenderPass renderPass,
    ...
    createPipeline(shaders, renderPass, usePBRVertex);
    SPARK_CORE_INFO("Graphics pipeline created.");
}
...
void Pipeline::createPipeline(const std::vector<std::shared_ptr<Shader>>& shaders, VkRenderPass renderPass, bool usePBRVertex) {
    // Shader stages

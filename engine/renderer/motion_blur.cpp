...
MotionBlurSystem::MotionBlurSystem(Device& device, uint32_t width, uint32_t height)
    : m_device(device), m_width(width), m_height(height) {

    createResources();
    SPARK_CORE_INFO("Motion blur system initialized: {0}x{1}", width, height);
}

MotionBlurSystem::~MotionBlurSystem() {
    vkDeviceWaitIdle(m_device.getDevice());
    cleanupResources();
    SPARK_CORE_INFO("Motion blur system destroyed.");
}

void MotionBlurSystem::recreate(uint32_t width, uint32_t height) {
    vkDeviceWaitIdle(m_device.getDevice());
    m_width = width;
    m_height = height;
...
void MotionBlurSystem::render(VkCommandBuffer commandBuffer, VkImageView colorView, VkImageView velocityView) {
...
void MotionBlurSystem::createResources() {
    // 创建输出纹理
    ...
    SPARK_CORE_INFO("Motion blur resources created.");
}

void MotionBlurSystem::cleanupResources() {
